...
class strategy_implement:
    def __init__(self, testnet, symbol, timeframe):
        ...
        self.take_profit: float = 0.02
        self.stop_loss: float = 0.01
    ...
    def new_candle(self, candle):
        ...
        if len(candle)==6:
            ...
            if candle[5]:
                ...
                logger.info(f"Current price: {self.current}, EMA: {self.ema.iloc[-1]}, rsi: {self.rsi.iloc[-1]}, macd: {self.macd_line.iloc[-1]}")
    ...
    def open_position(self,check_signal_result):
        ...
        self.filled_buy_order['price'] = self.current
        logger.warning('Placed buy order')       
    ...
    def order_trace(self,closed_candle):
        ...
        if closed_candle:
            ...
            if current_price==SL_price | current_price==TP_price | ema_sell | rsi_buy | macd_buy:
                ...
                logger.warning(f"Placed sell order after candle closed,return: {(current_price-buy_price)/buy_price}")
        ...
        if not closed_candle:
            if current_price==SL_price | current_price==TP_price:
                ...
                # except Exception as e:
                #     logger.error("Failed to set up sell orders, error: {e}")  
                 logger.warning(f"Placed sell order by hitting SL or TP, return: {(current_price-buy_price)/buy_price}")
